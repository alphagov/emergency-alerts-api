...
#                     "Slow delivery notifications detected for provider {}".format(provider_name)
#                 )
...
#             )  # noqa
#             ticket = NotifySupportTicket(
#                 subject=f"[{current_app.config['NOTIFY_ENVIRONMENT']}] High failure rates for sms spotted for services",  # noqa
#                 message=message,
#                 ticket_type=NotifySupportTicket.TYPE_INCIDENT,
#                 technical_ticket=True,
#             )
...
#     new_email_brands = (
#         EmailBranding.query.join(Organisation, isouter=True).filter(EmailBranding.created_at >= previous_weekday).all()  # noqa
#     )
