...
class CBCProxyClientBase(ABC):
    ...
    def _invoke_lambda(self, lambda_name, payload):
        ...
        try:
            current_app.logger.info(
                f"Calling lambda {lambda_name}", extra={"python_module": __name__, "payload": str(payload)[:400]}
            )
            ...
            current_app.logger.error(
                f"Boto ClientError calling lambda {lambda_name}", extra={"python_module": __name__, "error": str(error)}
            )
            success = False
            return success
