...
run_worker(){
    cd $DIR_API;
    export SERVICE=api_worker && . $VENV_API/bin/activate && exec dramatiq --skip-logging --processes 1 --threads 4 app.dramatiq.broker:broker --queues $WORKER_QUEUE_NAMES
}
...
run_periodiq(){
    ...
    export SERVICE=api_periodiq && . $VENV_API/bin/activate && exec python -m app.periodiq -v app.dramatiq.broker:broker
}
