...
def test_get_webauthn_credentials_errors_if_user_doesnt_exist(admin_request, sample_user):
    ...
    admin_request.get("webauthn.get_webauthn_credentials", user_id=uuid.uuid4(), _expected_status=404)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({"name": "my key", "credential_data": "ABC123"}, "registration_response is a required property"),
        # name is null
...
def test_create_webauthn_credential_errors_if_schema_violation(admin_request, sample_user, data, err_msg):
    response = admin_request.post(
        "webauthn.create_webauthn_credential", user_id=sample_user.id, _data=data, _expected_status=400
    )
